...
from rcb_cor_bits import sha256_custom, rcb_encrypt, rcb_decrypt, S, T
...
def test_sha256_custom_length():
    ...
    # transfrom data to bits
    data_bits = ''.join(format(byte, '08b') for byte in data)
    for tao in range(1, 129):
        h = sha256_custom(data_bits, tao)
        assert len(h) == tao
...
def test_invalid_tao_sigma_key(sample_key, sample_data):
    ...
    with pytest.raises(ValueError):
        rcb_encrypt(cipher, sample_data, sigma=-1, tao=16, key=sample_key)

    # invalid tao value
    with pytest.raises(ValueError):
        rcb_encrypt(cipher, sample_data, sigma=16, tao=-1, key=sample_key)
    
    # invalid key
    with pytest.raises(ValueError):
        rcb_encrypt(cipher, sample_data, sigma=32, tao=32, key=b"short_key")

    # sigma + tao > 16
    with pytest.raises(ValueError):
        rcb_encrypt(cipher, sample_data, sigma=80, tao=80, key=sample_key) 
